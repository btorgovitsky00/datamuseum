...
- Utility functions for data set management (`deduplicate`, `duplicate`,
  `italicize`).
- Example Octopodoidea occurrence data sets for Japan from the [Global
  ...
  data set obtained by direct request from the [National Museum of
  Nature and Science, Japan (NSMT)](https://www.kahaku.go.jp/english/).
- Example workflow for `datamuseum` with the article [Octopodoidea in
  ...
  Shows the pipeline for combining the accessioned data sets into one
  combined data set
